...
//
// Integra:
//   • Farm de XP (Duolingo-Lazy: lógica de sessions POST/PUT)
//   • Modo Super (Unlimited-Hearts: patch de JS via fetch+regex no background)
//   • Mensageria com content-script.js para resolver exercícios (Autolingo)
// ============================================================
  ...
  return new Promise((resolve) => {
    chrome.tabs.query({ url: '*://www.duolingo.com/*' }, (tabs) => {
      ...
      chrome.scripting.executeScript({
        ...
        func: () => {
          try {
            return document.cookie.split(';').find(c => c.includes('jwt_token'))?.split('=')[1] || null;
          } catch { return null; }
        }
      }, (results) => {
...
// Tenta reinjetar o content script na aba (caso a extenção tenha sido recarregada)
async function ensureContentScriptReady(tabId) {
  return new Promise((resolve) => {
    // Primeiro testa se o content script já responde
    chrome.tabs.sendMessage(tabId, { action: 'ping' }, (response) => {
      if (!chrome.runtime.lastError && response) {
        resolve(true);
        return;
      }
      // Não responde: tenta reinjetar
      chrome.scripting.executeScript({
        target: { tabId },
...
// Faz chamada HTTP via content script (necessário pq service worker não tem cookies)
async function duolingoFetchViaContentScript(url, method, headers, body) {
  return new Promise((resolve) => {
    chrome.tabs.query({ url: '*://www.duolingo.com/*' }, async (tabs) => {
      ...
      // Garante que o content script está ativo (reinjetando se necessário)
...
// ─── Farm de XP (Duolingo-Lazy: função Duolingo()) ───────────
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  // ── Fetch API proxy (do Duolingo-Lazy: chama API pelo content script)
  ...
  if (message.action === 'solve_challenge' || message.action === 'solve_skip_challenge' || message.action === 'set_delay') {
    chrome.tabs.query({ url: '*://www.duolingo.com/*', active: true }, (tabs) => {
      if (!tabs.length) {
        // tenta qualquer aba do Duolingo
        chrome.tabs.query({ url: '*://www.duolingo.com/*' }, (allTabs) => {
          if (allTabs.length) {
